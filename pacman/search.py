       ...
       if curr_state not in explored:
           ...
           for successor, action, _ in problem.getSuccessors(curr_state):
               if successor not in explored:
                   front.push((successor, actions + [action]))
   return []
...
def uniformCostSearch(problem):
    ...
    while not front.isEmpty():
        curr_state, actions, curr_cost = front.pop()
        ...
        if curr_state not in explored:
            ...
            for successor, action, step_cost in problem.getSuccessors(curr_state):
                if successor not in explored:
                    ...
                    front.push((successor, actions + [action], total_cost), total_cost)  
    ...
    while not front.isEmpty():
        ...
        if curr_state not in explored:
            ...
            for successor, action, step_cost in problem.getSuccessors(curr_state):
                ...
                if successor not in explored or new_cost < g_cost.get(successor, float('inf')):
                    g_cost[successor] = new_cost
                    f_cost = new_cost + heuristic(successor, problem)  
                   
                    front.push((successor, actions + [action], new_cost), f_cost)
